...
4. **Pick a model (optional).** Above the Diagnose Now button is a model dropdown — default is NIM Llama 3.3 70B (free). Stronger / different options: Llama 3.1 405B, Nemotron 70B (NVIDIA reasoning-tuned), Mixtral 8x22B, Gemini 1.5/2.0 Flash. For Gemini you'll need a Google API key (free tier from [aistudio.google.com](https://aistudio.google.com/app/apikey)) — paste it in the masked field, click Save. The key is stored only in your browser's localStorage.
...
6. **Read the structured advisory.** It will name specific XMV_X / XMEAS_Y tags. On Gemini, the output will be a structured *Top 3 candidate root causes / Most likely / Operator next steps* breakdown (the system prompt asks for that format; smaller models like Llama 70B may give a shorter version).
